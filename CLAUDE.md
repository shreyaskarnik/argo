...
Install invariants (verified against all 399 live registry manifests, Sept 2026): item files are fetched as `arrayBuffer` and written as raw bytes — never `res.text()`, which silently corrupts PNG/JPEG/WOFF2/WAV (a 74 KB PNG became 135 KB). File paths may be nested (`assets/sfx/x.wav`); every `/`-separated segment must match the item-name rule, plus a resolved-path check. A file with a `url` field is fetched from that URL (upstream hosts large assets on `static.heygen.ai`; nothing exists at the registry path) — `https://` only, 128 MB per file, all sources validated before the first write. Components are injected via `innerHTML` into the recorded page, so their relative `url()`/`src` references resolve against the app, not `blocksDir` — known limitation; blocks render from `file://` and are unaffected. When porting from upstream, put the upstream commit in the file's header comment (the original July ports lack one, so syncing them meant diffing by date; `gravitational-lens` records `dd06b321`). `tests/transitions/shader-render.test.ts` asserts every shader's first frame is the outgoing scene and last frame the incoming one.
